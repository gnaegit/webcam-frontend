...
export default function CameraStream() {
  ...
  const [cameraType, setCameraType] = useState<string | null>(null); // Allow null
  ...
  const handleWebSocketMessage = (message: MessageEvent) => {
    if (message.data instanceof Blob) {
      ...
      try {
        ...
        setCameraType(status.camera_type || null); // Update cameraType from WebSocket
        setCameraStatus(status.camera_status || { picamera: false, cameraids: false });
        console.log("WebSocket update:", status); // Debug log
      } catch (e) {
  ...
  const handleSelectCamera = async (value: string) => {
    try {
      ...
      setError(null);
      // WebSocket will update cameraType, so no need to set it here
    } catch (err) {
      setError(err instanceof Error ? err.message : "Unknown error occurred.");
      setFeedback(null);
    }
